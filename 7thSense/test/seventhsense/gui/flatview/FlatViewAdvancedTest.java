...
{
	private static final Logger LOGGER = Logger.getLogger(FlatViewAdvancedTest.class.getName());
	...
	public static INode getTestData()
	{
		...
		final FolderNode subNode3 = new FolderNode("Folder2");
		...
		final FolderNode subNode4 = new FolderNode("Folder3");
		subNode2.addNode(3, subNode4);
		return root;
	}
	...
	public static void main(final String[] args)
	{
		...
		EventQueue.invokeLater(new Runnable()
		{
			public void run()
			{
				try
				{
					final FlatViewAdvancedTest window = new FlatViewAdvancedTest();
	...
	{
		final INode testData = getTestData();
		...
		frame.getContentPane().add(splitPaneTop, BorderLayout.CENTER);

		final JSplitPane splitPane = new JSplitPane();
		splitPaneTop.setLeftComponent(splitPane);

		_tree = new TreeView();
		_tree.setModel(testData);
		...
		btnAdd.addActionListener(new ActionListener()
		{
			...
			{
				FlatViewAdvancedTest.this.addActionPerformed();
			}
		});
		...
		btnRemove.addActionListener(new ActionListener()
		{
			public void actionPerformed(final ActionEvent event)
			{
				FlatViewAdvancedTest.this.removeActionPerformed();
			}
		});
		...
		btnSetName.addActionListener(new ActionListener()
		{
			public void actionPerformed(final ActionEvent event)
			{
				FlatViewAdvancedTest.this.setNameActionPerformed();
			}
		});
		editorPanel.add(btnSetName);

		final JButton btnAddLeaf = new JButton("Add Leaf");
		btnAddLeaf.addActionListener(new ActionListener()
		{
			public void actionPerformed(final ActionEvent event)
			{
				final INode selection = _tree.getSelectedItem();
				selection.addNode(-1, new TestScenarioNode("New Scenario " + (_nodeIndex++)));
			}
		});
		editorPanel.add(btnAddLeaf);

		final FlatView flatView = new FlatView();
		flatView.setModel(testData);
		splitPaneTop.setRightComponent(flatView);
	}
